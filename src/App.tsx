  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      ...
      try {
        ...
        console.error(
          "[AuthStabilization] Initial getSession failed or timed out:",
          err,
        );
      }
      ...
      if (localStorage.getItem("mindflow_auto_updated") === "true") {
        ...
        setTimeout(() => {
          useNotificationStore.getState().showToast({
            title: "App Updated",
            message: (() => {
              const prev = localStorage.getItem("mindflow_previous_version");
              localStorage.removeItem("mindflow_previous_version");
              return prev
                ? `MindFlow updated successfully to v${import.meta.env.VITE_APP_VERSION || "unknown"}`
                : "Successfully updated! Resuming your quiz... 😎";
            })(),
            variant: "success",
          });
        }, 1000);
